...
FILENAME_RE = re.compile(
    r'^(?P<sample_id>HP\d+[A-Za-z]*)_(?P<force>[\d.]+)nN_(?P<run>R\d+)',
    re.IGNORECASE
)
def parse_xvg(xvg_path: Path):
    ...
    with xvg_path.open("r", encoding="utf-8", errors="ignore") as f:
        for line in f:
            ...
            if len(nums) >= 2:
                times.append(nums[0])
                vals.append(nums[-1])  # use last numeric column as distance
    return times, vals
...
def main():
    ...
    # parse all files and store (ordering keys, label, times, dists)
    ...
    # sort by sample_id, force, run
    series.sort(key=lambda t: (t[0][0], t[0][1], t[0][2]))

    # build rows: one time row, one distance row per label
    rows = []
